...
# Verifica se l'ora corrente è compresa tra le 5:01 e le 00:59
if [[ $hour -eq 1 || $hour -eq 2 || $hour -eq 3 || $hour -eq 4 || $hour -eq 5 ]]; then
  echo "$timestamp [info] L'ora corrente è compresa tra le 1:00 e le 05:59. Durante le ore di backup non si procede con il ripristino automatico " >> /var/log/myapp.log
  exit 0
fi
